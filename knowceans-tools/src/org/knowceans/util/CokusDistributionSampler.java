 ...
 * process. The random generator used is the thread-safe CokusRandom.
 * 
 * @author heinrich (partly adapted from Yee Whye Teh's npbayes Matlab / C code)
 */
public class CokusDistributionSampler extends CokusSampler implements
    Serializable {

    private static final long serialVersionUID = 5890981539691870638L;
    ...
    public static void main(String[] args) {
        ...
        CokusDistributionSampler rs = new CokusDistributionSampler();
     ...
     */
    public CokusDistributionSampler() {
        super();
    }
     ...
     * @param rand
     */
    public CokusDistributionSampler(long seed) {
        super(seed);
    }
     ...
     * @return
     */
    public final double randNorm(double mu, double sigma) {
        ...
        if (haveNextNextGaussian) {
            ...
            double v1, v2, s;
            do {
                v1 = 2 * randDouble() - 1; // between -1 and 1
                v2 = 2 * randDouble() - 1; // between -1 and 1
                s = v1 * v1 + v2 * v2;
            } while (s >= 1 || s == 0);
     ...
     * @return
     */
    public final double randGmm(double[] probs, double[] mean, double[] sigma) {
     ...
     * @return
     */
    public final double[] randGmm(int n, double[] probs, double[] mean,
        double[] sigma) {
     ...
     * @return
     */
    public final double[] randDmm(double[] probs, double[][] mean,
        double[] precision) {
     ...
     * @return
     */
    public final double[][] randDmm(int n, double[] probs, double[][] means,
        double[] precisions) {
    ...
    public final double[][] randDmm(int n, double[] probs, double[][] means,
        ...
        for (i = 0; i < n; i++) {
            double randNum = randDouble() * cumprobs[len - 1];
     ...
     * @return
     */
    public final double randBeta(double aa, double bb) {
     ...
     * @return
     */
    public final double randGamma(double rr) {
        double bb, cc, dd;
        ...
        if (rr <= 0.0) {
            ...
            /* Exponential */
            return -Math.log(randDouble());
        } else if (rr < 1.0) {
            /* Use Johnks generator */
            cc = 1.0 / rr;
            dd = 1.0 / (1.0 - rr);
            while (true) {
                xx = Math.pow(randDouble(), cc);
                yy = xx + Math.pow(randDouble(), dd);
                if (yy <= 1.0) {
                    assert yy != 0 && xx / yy > 0;
                    return -Math.log(randDouble()) * xx / yy;
                }
            }
        } else { /* rr > 1.0 */
            /* Use bests algorithm */
     ...
     * @param aa
     */
    public final double[] randGamma(double[] aa) {
     ...
     * @return
     */
    public final double randGamma(double shape, double scale) {
     ...
     * @return
     */
    public final int[] randPerm(int size) {
     ...
     * @return
     */
    public final int[][] randPerm(int size, int parts) {
     ...
     * @return
     */
    private final int[][] randParts(int[] items, int parts) {
     ...
     * @return
     */
    public final double[] randDir(double a, int dimension) {
     ...
     * @return
     */
    public final double[] randDir(double[] aa) {
     ...
     * @return
     */
    public final double[] randDir(double[] mean, double precision) {
     ...
     * @return
     */
    public final double[][] randDir(double[][] aa, int direction) {
        double[][] ww = null;
        if (direction == 1) {
     ...
     * @return
     */
    public final double[][] randDir(double[] aa, int repetitions) {
     ...
     */
    public final int[] randMultFreqs(double[] pp, int repetitions) {
     ...
     */
    public final int[] randMult(double[] pp, int repetitions) {
     ...
     * @return
     */
    public final int randMultSimple(final double[] pp) {
        ...
        double randNum = randDouble() * cumPp[i - 1];
    ...
    public final void testMult() {
        ...
        CokusDistributionSampler s = new CokusDistributionSampler();
     ...
     * normalisation.
     */
    public final int randMult(final double[] pp) {
     ...
     */
    public final int randMultDirect(double[] pp) {
        ...
        // this automatically normalises.
        double randNum = randDouble() * pp[i - 1];
        lastRand = randNum;
     ...
     */
    public final int randMultDirect(double[] pp, double randnum) {
     ...
     * @return
     */
    public final int binarySearch(double[] a, double p) {
        if (p < a[0]) {
            return 0;
        }
        int low = 0;
     ...
     * @param p
     */
    public final int randBinom(double N, double p) {
        int n = 0;
        for (int i = 0; i < N; i++) {
     ...
     */
    public final int randBernoulli(double p) {
        double a = randDouble();
        if (a < p) {
            return 1;
        }
     ...
     * @return
     */
    public final double randConParam(double alpha, int numgroup, int[] numdata,
     ...
     * @return
     */
    public final double randConParam(double alpha, int numdata, int numtopic,
    ...
    public final CrpData randCrp(double alpha, int numdata) {
     ...
     * @return
     */
    public final int randNumTable(double alpha, int numdata) {
        int ii, numtable;
     ...
     * @return
     */
    public final double[] randStick(double[] alpha, int numclass) {
     ...
     * @return
     */
    public final String randString(int length, byte[] alphabet) {
        if (alphabet == null)
        ...
        for (int k = 0; k < length; k++) {
            int i = (int) Math.floor(randDouble() * alphabet.length);
            pass[k] = alphabet[i];
        }
     ...
     * @return
     */
    public final double[] stirling(int nn) {
        if (allss[0] == null) {
     ...
     * @return
     */
    public final double randUniform(double numvalue) {
        return randDouble() * numvalue;
    }
     ...
     * @return
     */
    public final int randUniform(int numvalue) {
        return (int) Math.floor(randDouble() * numvalue);
    }
